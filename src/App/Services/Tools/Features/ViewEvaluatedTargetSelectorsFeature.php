...
use App\Domains\Minecraft\Tools\Tools\Other\EvaluateTargetSelectors;
...
class ViewEvaluatedTargetSelectorsFeature extends Feature
{
    public function handle(EvaluationRequest $request)
    {
        ...
        $tool = $this->run(GetToolJob::class, [
            'className' => EvaluateTargetSelectors::class,
            'inputs' => []
        ]);
        ...
        if (!$request->acceptsAnyContentType() && $request->acceptsHtml()) {
            ...
            return view("tools::pages/evaluators/target_selector", [
